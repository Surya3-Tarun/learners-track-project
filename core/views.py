...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if password1 != password2:
            ...
            login(request, user)
            messages.success(request, 'Account created! Welcome to Learners Track.')
            return redirect('home')
...
def chatbot(request):
    courses_data = list(Course.objects.values('title', 'category', 'level', 'price', 'description', 'duration', 'topics'))
    for c in courses_data:
        c['price'] = float(c['price'])  # Fix: Decimal is not JSON serializable
    total_courses = Course.objects.count()
    return render(request, 'core/chatbot.html', {'courses_data': json.dumps(courses_data), 'total_courses': total_courses})
...
def generate_bot_response(message, courses_qs):
    ...
    if any(w in message for w in ['ai', 'artificial intelligence', 'machine learning', 'ml']):
        ai_courses = courses_qs.filter(Q(category__icontains='AI') | Q(category__icontains='Machine') | Q(title__icontains='AI') | Q(title__icontains='Machine'))
    ...
    if any(w in message for w in ['web', 'frontend', 'backend', 'fullstack', 'html', 'css', 'javascript']):
        ...
        if web:
            return f"🌐 Web Development is one of our top categories!\n\n**{web.title}**\nLevel: {web.level} | Price: ₹{web.price}\n\n{web.description}\n\nWeb developers are among the highest-paid professionals!"

    return (f"🤔 I'm not sure about that specific query, but I can help you with:\n\n"
            f"• 📚 Course recommendations\n"
            f"• 💰 Pricing and duration info\n"
            f"• 🎯 Career guidance\n"
            f"• 🤖 AI/ML, Web Dev, Data Science courses\n\n"
            f"Try asking: 'Tell me about Python' or 'Best course for beginners' or 'List all courses'!")
